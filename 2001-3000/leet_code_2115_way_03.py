...
class Solution(object):
    def findAllRecipes(self, recipes, ingredients, supplies):
        ...
        while flag:
            flag = False
            recipe_queue_size = len(recipe_queue)
            for _ in range(recipe_queue_size):
                current_recipe = recipe_queue.popleft()
                if all(ingredient in available for ingredient in ingredients[current_recipe]):
                    result.append(recipes[current_recipe])
                    available.add(recipes[current_recipe])
                    flag = True
                else:
                    recipe_queue.append(current_recipe)
